...
table_name_stu_base_info = "stu_info.json"
...
class Student:
    def __init__(self, id):
        ...
        self.score_list = self.get_stu_score_list()  # 学生答题的分数
    ...
    def score_all_test(self):
        ...
        for score_file_name in score_file_name_list:
            ...
            # 根据配好的组合，解析数据，做好加减法
            self.make_stu_score(zip_score_res)  # 生成学生分数
            # 删除考试文件和考试数据
    ...
    def get_one_ask(self):
        ...
        # 190816新方案
        # if self.is_new_test:
        #     one_test = self.test_target
        if self.is_new_test:
            self.is_new_test = False
            one_test = random.choice(self.test_target)
            self.test_target.remove(one_test)

        else:
            self.is_new_test = True
            low_test_info = self.score_list
            lowest_test = low_test_info[0] if len(low_test_info) > 0 else None
            if lowest_test is None:
                return self.get_one_ask()
            # 用低分数据查出题目并返回
            one_test = self.select_test_dict_from_target(lowest_test)
    ...
    def ensure_test_target(self):
        ...
        return new_test_info_dict_list
    ...
    def get_stu_score_list(self):
        ...
        # 对分数进行排序
        skill_list.sort(key=lambda ele: ele["score"])
